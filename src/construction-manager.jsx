...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setWorkers(w);
      setReports(r);
      const ac = w.find(x => x._adminCode);
      if (ac) setAdminCode(ac._adminCode);
  ...
  const submitReport = async () => {
    ...
    // update project actualDays
    const daysMap = workerDaysForProject([...reports, saved], repProject);
    const total = Object.values(daysMap).reduce((a,b)=>a+b,0);
    const proj2 = projects.find(p => String(p.id) === String(repProject));
    if (proj2) {
      const updated = { ...proj2, actualDays: total };
  ...
  const LogoSmall = () => (
    <img src={LOGO_URL} alt="G&E" style={{ height:38, borderRadius:6, objectFit:"contain", background:"#fff", padding:3 }}/>
  );
  const LogoBig = () => (
    <div style={{ textAlign:"center", marginBottom:10 }}>
      <img src={LOGO_URL} alt="G&E Construction" style={{ height:110, objectFit:"contain", borderRadius:12 }}/>
    </div>
  );
  ...
  if (screen === "mLogin") return (
    <div style={{ ...base, background:"#1A1A2E", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ background:"#fff", borderRadius:20, padding:30, width:"100%", maxWidth:320, direction:"rtl" }}>
        ...
        <p style={{ margin:"12px 0 0", fontSize:11, color:"#BBB", textAlign:"center" }}>קוד נוכחי: {adminCode}</p>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ ...base, background:"#F5F5F0" }}>
      ...
      <div style={{ background:"#fff", borderBottom:"1.5px solid #EEE", display:"flex", justifyContent:"center" }}>
        {tabs.map(t => (
          <button key={t.key} onClick={()=>{ setMgTab(t.key); setDetailId(null); }} style={{ background:"none", border:"none", borderBottom:mgTab===t.key?"3px solid #E8C547":"3px solid transparent", padding:"11px 14px", fontWeight:mgTab===t.key?700:500, fontSize:13, cursor:"pointer", fontFamily:"Heebo,sans-serif", color:mgTab===t.key?"#1A1A2E":"#888" }}>
            {t.emoji} {t.label}
          </button>
        ))}
      </div>
          ...
          <>
            ...
            {reports.length===0 && (
              <div style={{ background:"#fff", borderRadius:14, padding:44, textAlign:"center", border:"1.5px dashed #DDD", color:"#AAA" }}>
                ...
                <p style={{ margin:0 }}>אין דיווחים עדיין</p>
              </div>
            )}
          ...
          <>
            ...
            {projects.length===0 && (
              <div style={{ background:"#fff", borderRadius:14, padding:44, textAlign:"center", border:"1.5px dashed #DDD", color:"#AAA" }}>
                ...
                <p style={{ margin:0 }}>אין פרויקטים — הוסף את הראשון</p>
              </div>
            )}
            {projects.map(p => {
              const sc = STATUS_COLORS[p.status]||STATUS_COLORS["ממתין"];
              const pr = projReports(p.id);
              const daysMap = workerDaysForProject(reports, p.id);
            ...
            <>
              ...
              <div style={{ background:"#fff", borderRadius:14, padding:"16px 20px", marginBottom:14, boxShadow:"0 2px 8px rgba(0,0,0,0.07)" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                  <h3 style={{ margin:0, fontSize:15, fontWeight:700 }}>👷 ימי עבודה לפי עובד</h3>
                  <span style={{ background:"#F0F4FF", color:"#3B5BDB", borderRadius:8, padding:"4px 12px", fontSize:13, fontWeight:700 }}>סה"כ: {totalDays} ימים</span>
                </div>
        ...
        {mgTab==="workers" && (
          <>
            ...
            {workers.length===0 && (
              <div style={{ background:"#fff", borderRadius:14, padding:44, textAlign:"center", border:"1.5px dashed #DDD", color:"#AAA" }}>
                <div style={{ fontSize:34, marginBottom:8 }}>👷</div>
                <p style={{ margin:0 }}>אין עובדים — הוסף את הצוות</p>
              </div>
            )}
        ...
        {mgTab==="settings" && (
          <>
            ...
            <div style={{ background:"#fff", borderRadius:14, padding:22, boxShadow:"0 2px 8px rgba(0,0,0,0.07)", maxWidth:360 }}>
              ...
              <p style={{ margin:"0 0 12px", color:"#777", fontSize:13 }}>קוד נוכחי: <strong>{adminCode}</strong></p>
